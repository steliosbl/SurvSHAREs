...
def get_n_shapes(pogram_str, categorical_variables=[]):
    program = pogram_str.replace('(',',')
    ...
    for node in program_list:
        if node == 'shape':
            res += 1
        elif node.isnumeric() and prev_node != 'shape':
            if int(node) in categorical_variables:
                res += 1
        prev_node = node
    return res

def get_n_variables(pogram_str):
    program = pogram_str.replace('(',',')
    program = program.replace(')','')
    program = program.replace("'","")
    program = program.replace(" ","")
